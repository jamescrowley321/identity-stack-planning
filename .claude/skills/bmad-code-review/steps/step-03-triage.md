...
1. **Normalize** findings into a common format. Expected input formats:
   - Adversarial (Cold Read): markdown list of descriptions
   - Edge Cases: JSON array with `location`, `trigger_condition`, `guard_snippet`, `potential_consequence` fields
   - Acceptance Criteria: markdown list with title, AC/constraint reference, and evidence
   ...
   - `source` -- `cold_read`, `edge`, `auditor`, or merged sources (e.g., `blind+edge`)
