...
**AC 3 — Issuer mismatch rejection**
...
**AC 7 — Integration tests (node-oidc-provider)**
...
## Story 3 — JWKS Client + Key Resolution
...
**AC 5 — Key rotation (forced refresh)**
...
**AC 8 — Integration tests (node-oidc-provider)**
...
**AC 5 — Temporal claims (`exp`, `nbf`, `iat`)**
...
**AC 6 — Nonce validation (ID tokens)**
...
**AC 9 — Integration tests (node-oidc-provider)**
...
## Story 5 — Client Credentials + Authorization Code + PKCE Flows
...
**AC 2 — Client authentication methods**
...
**AC 3 — PKCE code_verifier generation**
...
**AC 4 — PKCE code_challenge generation**
...
**AC 5 — Authorization Code exchange**
...
**AC 6 — Token response validation**
...
> **Then** valid tokens are obtained and can be validated using Story 4's `validateToken()`.
...
## Story 6 — UserInfo Endpoint Client
...
**AC 5 — Bearer token via POST body**
...
> **Given** a running `node-oidc-provider` instance and a valid access token obtained via Story 5,
> **When** `fetchUserInfo()` is called,
