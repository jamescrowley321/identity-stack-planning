...
|---|---|---|---|
| Python | `py-identity-model` (PyPI, v3.18.x, certified RP) | ~16 library env-read sites across `ssl_config.py`, `core/http_utils.py`, `core/jwks_cache.py`; typed `OIDCSettings` (`OIDC_*` prefix) exists only in the `fastapi-identity-model` sub-package | Full: Config + `ConfigSource` Protocol + EnvSource; absorb the ad-hoc reads; align/absorb `OIDCSettings` |
