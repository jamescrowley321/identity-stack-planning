...
2. Write tests for ALL new code:
   ...
   - **Fail-closed test for every security control (mandatory).** For any control that rejects bad input (signature / alg / aud / iss / exp checks, sender-constraint binding `cnf.x5t#S256` / `cnf.jkt`, `algorithms` allowlist enforcement, secret redaction), write a test that FAILS if the control is removed: delete the control line locally, confirm the test goes red, then restore it. A control with no such mutation test is treated as **not implemented**. For py-identity-model put these under `src/tests/security/`. This is not self-attested — it is **verified mechanically by mutation testing** (`make mutation-security`, Epic 19): a surviving mutant on a security line means no test catches its removal and the gate fails. See `RED-BLUE-GATE.md`.
