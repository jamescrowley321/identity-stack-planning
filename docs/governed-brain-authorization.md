...
| Callback `iss` (RFC 9207) + `state` validation | Closes authorization-response mix-up and login-CSRF before a client can obtain a grant | Go and Rust clients lack a callback builder/parser surface for this | Federation-pilot gate; add executable callback vectors before adoption |
| Discovery endpoint-authority binding | Stops a malicious discovery document redirecting key or token requests | Not enforced consistently across implementations | Pre-federation security gate, with explicit alias and loopback exceptions |
| Bounded discovery/JWKS caches, single-flight fetch | Limits memory growth and duplicate key fetches during rotation or cache miss | Cache-entry limits absent in Go and Rust paths; Rust also lacks concurrent-fetch dedup | Implement before multi-issuer federation; publish staleness and failure semantics |
