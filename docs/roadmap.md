...
- Rust security hardening **DONE** (merged PRs #37/#39; jsonwebtoken now v11); conformance OIDF RP-harness in-flight (Go RP passes `basic-rp` — K1 #43 / K2 #44 merged; K3–K6 open). **Priorities reconciled 2026-08-12** ([`docs/identity-model-reconciliation-2026-08-12.md`](identity-model-reconciliation-2026-08-12.md)). Next: Parity-Hardening Sweep [in-session], then the Rust Extended-tier loop (`ralph-prompts/identity-model-rust-extended.md`), then Conformance K3–K6 — one workstream per repo at a time
